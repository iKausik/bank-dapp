...
function App() {
  ...
  const contractAddress = "0x8496a9661E410D4538FcfeF0716B576D53c0af8C";
  ...
  const getBankName = async () => {
    try {
      if (window.ethereum) {
        ...
        setError("Please install a Metamask wallet to useour bank.");
      }
    } catch (error) {
      console.log(error);
    }
  ...
  const depositMoneyHandler = async (event) => {
    try {
      ...
      if (window.ethereum) {
        ...
        await txn.wait();
        console.log("Deposit money...done", txn.hash);
